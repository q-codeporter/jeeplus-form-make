  ...
  <div v-if="show">
    <el-form label-position="top" size="small">
      ...
      <el-form-item v-if="bindDataTable" label="表名" prop="tableName">
                <el-select v-if="parent.type !== 'table'" v-model="data.tableName" style="width:100%" @change="clearModel" filterable placeholder="请选择表">
                  <el-option
                ...
                <el-select v-else v-model="parent.tableName" :disabled="true" style="width:100%" @change="clearModel" filterable placeholder="请选择表">
                  <el-option
                    v-for="item in dataTables"
                    :key="item.name"
                    :label="item.nameAndComments"
    ...
    <el-card class="box-card"  v-if="bindDataTable && mainTable !== data.tableName && parent.type !== 'table'">
      <div slot="header" class="clearfix">
        <span>请配置表关联字段</span>
      </div>
       <el-form-item :label="`主表[${mainTable}]:`" style="border-bottom:0px;padding-bottom:0px">
        <el-select  v-model="data.primaryKey"  size="mini" filterable placeholder="请选择字段">
                  <el-option
            ...
            <el-tag>等于</el-tag>
      <el-form-item :label="`附表[${data.tableName}]:`" style="border-bottom:0px;padding-bottom:0px">
            <el-select  v-model="data.foreignKey"  size="mini" filterable placeholder="请选择字段">
                  <el-option
      ...
      <el-form-item  :label="$t('fm.config.widget.model')"  v-if="data.type != 'td' && data.type != 'col'" >
            <!--如果是子表，且子表绑定数据库中的附表-->
            <el-input v-if="bindDataTable && mainTable !== data.tableName && data.type==='table' " :disabled="true" v-model="data.model"></el-input>
        
            <!--如果是子表，且绑定主表中的字段-->
             <el-select v-else-if="bindDataTable && mainTable === data.tableName && data.type==='table'" v-model="data.model"  style="width:100%" clearable filterable placeholder="请选择字段">
                  <el-option
                    v-for="item in dataTableColumns"
                    :key="item.name"
                    :label="item.nameAndComments"
            ...
            <!--如果子表中的元素，且子表是附表-->
            <el-select v-else-if="bindDataTable&& data.type!=='table'&&parent.type==='table'&&parent.tableColumns" v-model="data.model"  style="width:100%" filterable clearable placeholder="请选择字段">
                  <el-option
            ...
            <!--如果是主表中的元素-->
             <el-select v-else-if="bindDataTable&& data.type!=='table'" v-model="data.model"  style="width:100%" filterable clearable placeholder="请选择字段">
                  <el-option
                    v-for="item in dataTableColumns"
                    :key="item.name"
                    :label="item.nameAndComments"
            ...
            <el-input v-else  :disabled="data.type =='grid' || data.type == 'tabs' || data.type == 'report' || data.type == 'divider'" v-model="data.model"></el-input>
      </el-form-item>
      <el-form-item :label="$t('fm.config.widget.name')" v-if="data.type!='grid' && data.type != 'tabs' && data.type != 'report' && data.type != 'td' && data.type != 'col'">
      ...
      <!--hideLabel-->
      <el-form-item :label="$t('fm.config.widget.hideLabel')" v-if="data.type !='grid' && data.type != 'tabs' && data.type != 'report' && data.type != 'divider' && data.type != 'td' && data.type != 'col'">
      ...
      <el-form-item :label="$t('fm.config.widget.dataUrl')" v-if="data.type=='selectTree'">
      ...
      <el-form-item :label="$t('fm.config.widget.multiple')" v-if="data.type=='select' || data.type=='imgupload' || data.type == 'fileupload'">
        <el-switch v-model="data.options.multiple" @change="handleSelectMuliple"></el-switch>
            ...
            <el-form-item :label="$t('fm.config.widget.multiple')" v-if="data.type=='selectTree'">
        <el-switch v-model="data.options.checkbox"></el-switch>
      </el-form-item>
      <el-form-item :label="$t('fm.config.widget.isOnlySelectLeaf')" v-if="data.type=='selectTree'">
      ...
      <el-form-item :label="$t('fm.config.widget.option')" v-if="Object.keys(data.options).indexOf('options')>=0">
        <el-radio-group v-model="data.options.remote" size="mini" style="margin-bottom:10px;">
          <el-radio-button :label="1">{{$t('fm.config.widget.staticData')}}</el-radio-button>
          <el-radio-button :label="2">{{$t('fm.config.widget.remoteData')}}</el-radio-button>
          <el-radio-button :label="3">{{$t('fm.config.widget.dictData')}}</el-radio-button>
        </el-radio-group>
        ...
        <template v-else-if="data.options.remote === 3">
          <el-form size="small" :inline="true"  label-width="70px">
            ...
            <el-form-item label="默认值:" style="border-bottom: 0">
              ...
              <el-checkbox-group v-if="data.type=='checkbox'" v-model="data.options.defaultValue" 
                >
                  <el-checkbox
                    
                    :style="{display: data.options.inline ? 'inline-block' : 'block'}"
                    :label="item.value" v-for="(item, index) in $dictUtils.getDictList(data.options.dictType)" :key="index"
                  >
                  {{item.label}}
                ...
                <el-radio-group 
                  ...
                  >
                    <el-radio  
                      :style="{display: data.options.inline ? 'inline-block' : 'block'}"
                      :label="item.value" v-for="(item, index) in $dictUtils.getDictList(data.options.dictType)" :key="item.value + index"
                    >
                      {{ item.label}}
                    </el-radio>

                  </el-radio-group>
                  <el-button size="mini"   v-if="data.type=='radio'" @click="data.options.defaultValue=''">清空</el-button>
                  <el-select
        ...
        <template v-else>
          <div v-if="Object.keys(data.options).indexOf('showLabel')>=0">
            <el-checkbox size="mini" v-model="data.options.showLabel">{{$t('fm.config.widget.showLabel')}}</el-checkbox>
          </div>
            ...
            <el-radio-group v-model="data.options.defaultValue">
              ...
              >
                <li v-for="(item, index) in data.options.options" :key="index" >
                  ...
                  >
                    <el-input clearable :style="{'width': data.options.showLabel? '90px': '180px' }" size="mini" v-model="item.value"></el-input>
...
<el-form-item :label="$t('fm.config.widget.defaultValue')" 
        v-if="Object.keys(data.options).indexOf('defaultValue')>=0 && data.type != 'custom' && data.type != 'imgupload' && data.type != 'fileupload'">
        <el-input clearable v-if="data.type=='textarea'" type="textarea" :rows="5" v-model="data.options.defaultValue"></el-input>
        
        <template v-if="data.type=='input'  || data.type == 'text' ||  data.type == 'user' || data.type == 'office'">
        ...
        <template v-if="data.type == 'editor'">
          <el-button style="width: 100%;" @click="editorVisible = true">{{$t('fm.config.widget.setting')}}</el-button>
          <cus-dialog 
        ...
        <template v-if="data.type == 'table'">
          <el-button style="width: 100%;" @click="tableVisible = true">{{$t('fm.config.widget.setting')}}</el-button>
          <cus-dialog 
      ...
      <template v-if="data.type == 'component'">
        <el-form-item :label="$t('fm.config.widget.customTemplates')">
          ...
          <el-button size="small" style="width: 100%;" @click="handleSetTemplate">{{$t('fm.config.widget.setting')}}</el-button>
        </el-form-item>
      </template>
        ...
        <el-form-item :label="$t('fm.config.widget.tabOption')">
          ...
          >
            <li v-for="(item, index) in data.tabs" :key="index" >
              <i class="drag-item" style="font-size: 16px;margin: 0 5px;cursor: move;"><i class="iconfont icon-icon_bars"></i></i>
              <el-input  clearable :placeholder="$t('fm.config.widget.tabName')" size="mini" style="width: 200px;" v-model="item.label"></el-input>
      ...
      <el-form-item :label="$t('fm.config.widget.customClass')" v-if="Object.keys(data.options).includes('customClass') && !data.options.tableColumn">
      ...
      <template v-if="data.type != 'grid' && data.type != 'tabs' && data.type != 'report' && data.type != 'divider' && data.type != 'td' && data.type != 'col'">
        ...
        <el-form-item :label="$t('fm.config.widget.validate')">
          ...
          <div class="validate-block" v-if="Object.keys(data.options).indexOf('pattern')>=0">
            ...
            <el-input clearable :disabled="!data.options.patternCheck" size="mini" v-model.lazy="data.options.pattern"  style=" width: 239px;" :placeholder="$t('fm.config.widget.patternPlaceholder')"></el-input>
...
export default {
  ...
  created () {
     ...
     this.dictService.list({ 'current': 1,
          'size': -1}).then(({data}) => {
        this.dictList = data.records
      })
      if(this.bindDataTable){
        this.makeFormService.getTableList({'dataSource.enName': this.data?.enName??'master'}).then(({data}) => {
  ...
  methods: {
    clearModel () {
      ...
      querySearch(queryString, cb) {
        ...
        var results = [{title:"姓名", value: '${user.name}'},
          ...
          if(this.bindDataTable && newVal){
            if(this.mainTable !== newVal){
               let fieldArra = this.dataBindFields.filter((field)=>{
                ...
                this.makeFormService.getTableColumnList({name: newVal, 'dataSource.enName': this.data?.enName??'master'}).then(({data}) => {
